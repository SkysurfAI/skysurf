"""VAL-ENGINE-B-EXPERIMENTAL: Portfolio Simulation Engine (Phase 4).

Copy of val_engine_b.py with multi-dimensional regime gating:
  - Overall regime (breadth-based or weinstein)
  - Sector regime (from sector_regime_weekly.csv)
  - Cap-tier regime (from captier_regime_weekly.csv)

When regime_combination="OVERALL_ONLY" with default breadth settings,
produces EXACTLY the same results as the locked Gladiator.

Usage:
    Import and call run_simulation() from run_coarse_sweep.py
"""
...
# matplotlib is needed only for the optional chart generators (generate_charts
# and the engine's own __main__). It is imported lazily through this proxy so
# the reproduction backtest runs with the base `pip install skysurf` and does
# not require matplotlib. Install `skysurf[reproduction]` to enable charts.
class _LazyPlt:
    _plt = None
...
# ── Paths ─────────────────────────────────────────────────────────────
# Data paths resolve from the reproduction data bundle (see skysurf.reproduction
# ._paths). ENGINE_A_DIR and ENGINE_A_V2_DIR collapse to a single flat data dir.
BASE_DIR = Path(__file__).resolve().parent
ENGINE_A_DIR = _paths.data_dir()
ENGINE_A_V2_DIR = _paths.data_dir()
STOCK_CACHE_DIR = ENGINE_A_DIR / "stock_weekly_cache"
OUTPUT_DIR = _paths.output_dir() / "val_engine_b_experimental_results"
...
CONFIG = {
    ...
    "stop_method": "CURRENT",        # "CURRENT" or "STRUCTURAL"
    "stop_atr_buffer": 1.0,          # ATR multiplier for structural stop
    "stop_atr_by_captier": False,    # scale buffer by cap-tier

    # Volume confirmation (Phase 4)
    "volume_min_ratio": None,        # min volume_ratio to allow entry (None = off)

    # Pyramiding (Phase 4)
    "pyramid_enabled": False,
    "pyramid_initial_risk_pct": 0.005,   # probe: half of normal 1%
    "pyramid_add_trigger_atr": 1.0,      # add after stock moves up 1×ATR
    "pyramid_add_risk_pct": 0.005,       # second tranche risk
    "pyramid_add_sector_check": False,   # check sector regime+RS before add
    "pyramid_stop_on_add": "BREAKEVEN",  # "BREAKEVEN" or "STRUCTURAL"
    ...
    # Mechanism A: Time Stop (cut non-starters)
    "time_stop_enabled": False,
    "time_stop_weeks": 12,
    "time_stop_min_return_pct": 5.0,
    "time_stop_action": "EXIT",           # "EXIT" or "TIGHTEN"
    "time_stop_tighten_pct": 3.0,

    # Mechanism B: Partial Profit (lock in gains)
    "partial_profit_enabled": False,
    "partial_profit_trigger_pct": 30.0,
    "partial_profit_sell_pct": 33.0,
    "partial_profit_move_stop": "NONE",   # "NONE" / "BREAKEVEN" / "HALF_BACK"

    # Mechanism C: Progressive Trail Tightening (protect profits)
    "trail_tighten_enabled": False,
    "trail_tighten_trigger_pct": 30.0,
    "trail_tighten_ma_type": "SMA",
    "trail_tighten_ma_period": 20,
    "trail_tighten_atr_buffer": 0.5,
    ...
    "cost_model": "FLAT",  # "FLAT" (legacy slippage+brokerage) or "ZERODHA" (real Indian delivery)
...
# ── Multi-regime combination modes (Phase 4) ─────────────────────────
REGIME_COMBINATIONS = {
    "OVERALL_ONLY", "SECTOR_ONLY", "CAPTIER_ONLY",
    "OVERALL_OR_SECTOR", "OVERALL_OR_CAPTIER",
    "SECTOR_AND_CAPTIER",
    "OVERALL_OR_SECTOR_AND_CAPTIER",
    "ANY_TWO_OF_THREE",
}


# ── Cost models ───────────────────────────────────────────────────────
# FLAT:    legacy slippage_pct + brokerage_pct, applied per-side as a percentage.
# ZERODHA: real Indian equity-delivery — zero brokerage, regulatory percentages
#          (STT + NSE txn + SEBI + stamp + GST) plus a flat Rs 15.93 DP charge
#          on every sell transaction (per scrip per sell, including partials).

_ZERODHA_BUY_PCT = 0.0011874   # STT 0.10% + NSE txn 0.00307% + SEBI 0.0001% + stamp 0.015% + GST on (txn+SEBI)
_ZERODHA_SELL_PCT = 0.0010374  # STT 0.10% + NSE txn 0.00307% + SEBI 0.0001% + GST on (txn+SEBI); no stamp on sell
_ZERODHA_DP_FLAT = 15.93       # Rs per sell transaction per scrip (Rs 13.50 + 18% GST)


def _buy_cost_factor(cfg: dict) -> float:
    """Per-share buy multiplier (price → effective per-share outflow). Inverse used to back out entry price."""
    slip = cfg.get("slippage_pct", 0.001)
    if cfg.get("cost_model", "FLAT") == "ZERODHA":
        return 1 + _ZERODHA_BUY_PCT + slip
    return 1 + slip + cfg.get("brokerage_pct", 0.0011)


def _buy_cost(qty: float, price: float, cfg: dict) -> float:
    """Total cash outflow for a buy transaction."""
    return qty * price * _buy_cost_factor(cfg)


def _sell_proceeds(qty: float, price: float, cfg: dict) -> float:
    """Cash inflow from a sell transaction (price-side fees + DP charge)."""
    slip = cfg.get("slippage_pct", 0.001)
    if cfg.get("cost_model", "FLAT") == "ZERODHA":
        return qty * price * (1 - _ZERODHA_SELL_PCT - slip) - _ZERODHA_DP_FLAT
    return qty * price * (1 - slip - cfg.get("brokerage_pct", 0.0011))
...
class Position:
    ...
    # Pyramid tracking
    pyramid_status: str = "full"         # "probe" or "full"
    pyramid_trigger_price: float = 0.0   # entry + trigger_atr × atr
    pyramid_atr_at_entry: float = 0.0    # ATR at entry (for trigger calc)
    pyramid_add_date: Any = None         # when add happened
    pyramid_original_qty: int = 0        # qty before add
    pyramid_original_cost: float = 0.0   # cost before add
    pyramid_add_price: float = 0.0       # raw price of add tranche
...
def is_entry_allowed(regime: str, breadth_pct: float | None, cfg: dict) -> bool:
    ...
    Uses cfg["overall_breadth_threshold"] (default 65) instead of a hardcoded
    threshold for the breadth gate in aggressive filters. This makes the
    threshold a tunable sweep parameter.

    Note: with breadth_5state, the 5-state classifier assigns recovering/
    deteriorating to breadth in [35%, 60%). So threshold >= 60 effectively
    blocks all recovering/deteriorating (same as original hardcoded 65).
    Threshold < 60 allows some recovering/deteriorating weeks.
    """
    rt = cfg["regime_type"]
    ef = cfg["entry_filter"]
...
# ── Multi-Regime Data Loaders (Phase 4) ──────────────────────────────

def load_sector_regime(path: Path | None = None) -> dict[tuple, str]:
    """Load sector_regime_weekly.csv → dict[(week_date_str, index_symbol)] → regime."""
    if path is None:
        path = ENGINE_A_V2_DIR / "sector_regime_weekly.csv"
    if not path.exists():
        return {}
    df = pd.read_csv(path)
    lookup = {}
    for _, row in df.iterrows():
        key = (str(row["week_date"])[:10], row["index_symbol"])
        lookup[key] = row["regime"]
    return lookup


def load_captier_regime(path: Path | None = None) -> dict[tuple, str]:
    """Load captier_regime_weekly.csv → dict[(week_date_str, cap_tier)] → regime."""
    if path is None:
        path = ENGINE_A_V2_DIR / "captier_regime_weekly.csv"
    if not path.exists():
        return {}
    df = pd.read_csv(path)
    lookup = {}
    for _, row in df.iterrows():
        key = (str(row["week_date"])[:10], row["cap_tier"])
        lookup[key] = row["regime"]
    return lookup


def load_ticker_metadata(path: Path | None = None) -> dict[str, dict]:
    """Load ticker_metadata.csv → dict[ticker] → {primary_sector, index_symbol, cap_tier, ...}."""
    if path is None:
        path = ENGINE_A_V2_DIR / "ticker_metadata.csv"
    if not path.exists():
        return {}
    df = pd.read_csv(path)
    lookup = {}
    for _, row in df.iterrows():
        lookup[row["ticker"]] = {
            ...
            "cap_tier": row.get("cap_tier", "MICRO"),
            "sector_has_index_data": bool(row.get("sector_has_index_data", False)),
        }
    return lookup


def load_sector_rs_quartile(path: Path | None = None) -> dict[tuple, str]:
    """Load sector_regime_weekly.csv → dict[(week_date_str, index_symbol)] → quartile."""
    if path is None:
        path = ENGINE_A_V2_DIR / "sector_regime_weekly.csv"
    if not path.exists():
        return {}
    df = pd.read_csv(path)
    if "sector_rs_quartile" not in df.columns:
        return {}
    lookup = {}
    for _, row in df.iterrows():
        q = row.get("sector_rs_quartile")
        if pd.notna(q):
            key = (str(row["week_date"])[:10], row["index_symbol"])
            lookup[key] = str(q)
    return lookup
...
def passes_sector_rs_filter(quartile: str | None, cfg: dict) -> bool:
    """Check if a sector's RS quartile passes the configured filter."""
...
def is_entry_allowed_multi_regime(
    ...
    """Apply multi-dimensional regime gate.

    Args:
        overall_allowed: Result of original is_entry_allowed() for this week.
        sector_regime: "bull"/"bear"/"sideways" or None (from sector_regime_weekly).
        captier_regime: "bull"/"bear"/"sideways" or None (from captier_regime_weekly).
        sector_has_data: Whether this ticker's sector has index data.
        cfg: Config dict with 'regime_combination' key.
    """
    ...
    sector_bull = (sector_regime == "bull") if sector_has_data and sector_regime else None
    captier_bull = (captier_regime == "bull") if captier_regime else False

    if combo == "SECTOR_ONLY":
        if sector_bull is None:
            return overall_bull  # fallback
        return sector_bull
    ...
    if combo == "OVERALL_OR_SECTOR":
        if sector_bull is None:
            return overall_bull
        return overall_bull or sector_bull
    ...
    if combo == "SECTOR_AND_CAPTIER":
        if sector_bull is None:
            return overall_bull and captier_bull  # fallback
        return sector_bull and captier_bull
    ...
    return overall_allowed  # unknown combo → baseline
...
def _build_trade(pos: Position, exit_price: float, exit_date, exit_reason: str,
    ...
    return {
        ...
        # Pyramid fields
        "pyramid_status": pos.pyramid_status,
        "pyramid_add_date": str(pos.pyramid_add_date.date()) if pos.pyramid_add_date is not None else None,
        "pyramid_add_price": round(pos.pyramid_add_price, 2) if pos.pyramid_add_price > 0 else None,
        "pyramid_original_qty": pos.pyramid_original_qty if pos.pyramid_original_qty > 0 else None,
        "pyramid_original_cost": round(pos.pyramid_original_cost, 2) if pos.pyramid_original_cost > 0 else None,
    }


# ── Per-week extracted functions (for brain wrapping) ────────────────
# These are MECHANICAL extractions of the exit and entry blocks from
# run_simulation's main loop. Bodies are copied verbatim — no logic
# changes. Closure references became explicit parameters; positions
# and cash are returned (since reassignment doesn't propagate to caller).
# closed_trades and diag are mutated in-place via the passed list/dict.

def evaluate_exits_for_week(
    wk,
    ...
    """Per-week exit evaluation. Returns (updated_positions, updated_cash).

    closed_trades and diag are mutated in-place. Direct extraction of
    val_engine_b_experimental.py exit block (was lines 885-1080).
    """
    ...
    for pos in positions:
        ...
        # ── Mechanism C: Progressive Trail Tightening ────────
        if cfg.get("trail_tighten_enabled") and atr_val is not None and not pd.isna(atr_val):
            _unreal_tt = (week_close - pos.entry_price) / pos.entry_price * 100 if pos.entry_price > 0 else 0
            if _unreal_tt >= cfg["trail_tighten_trigger_pct"] and not pos.trail_tightened:
                pos.trail_tightened = True
                diag["trail_tighten_triggered"] += 1
            if pos.trail_tightened:
                _tt_col = f"{cfg['trail_tighten_ma_type'].lower()}_{cfg['trail_tighten_ma_period']}"
                _tt_ma = row.get(_tt_col)
                if _tt_ma is not None and not pd.isna(_tt_ma):
                    _tt_trail = float(_tt_ma) - cfg["trail_tighten_atr_buffer"] * float(atr_val)
                    pos.stop_level = max(pos.stop_level, _tt_trail)

        # Update MAE tracking (always, even on exit week)
        pos.min_low = min(pos.min_low, week_low)

        # ── Mechanism A: Time Stop ───────────────────────────
        forced_exit = False
        forced_exit_reason = ""
        if cfg.get("time_stop_enabled"):
            _hold_ts = max(1, round((wk - pos.entry_date).days / 7))
            _unreal_ts = (week_close - pos.entry_price) / pos.entry_price * 100 if pos.entry_price > 0 else 0
            if (_hold_ts >= cfg["time_stop_weeks"]
                    and _unreal_ts < cfg["time_stop_min_return_pct"]
                    and not pos.time_stop_triggered):
                if cfg["time_stop_action"] == "EXIT":
                    forced_exit = True
                    forced_exit_reason = "time_stop_exit"
                    diag["time_stop_exits"] += 1
                elif cfg["time_stop_action"] == "TIGHTEN":
                    _tl = pos.entry_price * (1 - cfg["time_stop_tighten_pct"] / 100)
                    pos.stop_level = max(pos.stop_level, _tl)
                    pos.time_stop_triggered = True
                    diag["time_stop_tightens"] += 1
                elif cfg["time_stop_action"] in ("RANGE_LOW_05", "RANGE_LOW_025", "EMA_TRAIL"):
                    # Structural tightening: use stock's actual price structure
                    _ts_weeks = cfg["time_stop_weeks"]
                    _entry_idx = sdf.index.get_loc(pos.entry_date) if pos.entry_date in sdf.index else -1
                    if _entry_idx >= 0:
                        _start_idx = max(_entry_idx, sdf.index.get_loc(wk) - _ts_weeks)
                        _end_idx = sdf.index.get_loc(wk)
                        _atr_ts = float(atr_val) if atr_val is not None and not pd.isna(atr_val) else 0
                        if cfg["time_stop_action"] == "RANGE_LOW_05":
                            _range_low = float(sdf.iloc[_start_idx:_end_idx + 1]["Low"].min())
                            _struct_stop = _range_low - 0.5 * _atr_ts
                        elif cfg["time_stop_action"] == "RANGE_LOW_025":
                            _range_low = float(sdf.iloc[_start_idx:_end_idx + 1]["Low"].min())
                            _struct_stop = _range_low - 0.25 * _atr_ts
                        else:  # EMA_TRAIL
                            _ema20_val = row.get("ema_20")
                            _struct_stop = float(_ema20_val) - 0.5 * _atr_ts if _ema20_val is not None and not pd.isna(_ema20_val) else pos.stop_level
                        if _struct_stop > 0:
                            pos.stop_level = max(pos.stop_level, _struct_stop)
                    pos.time_stop_triggered = True
                    diag["time_stop_tightens"] += 1

        # ── Mechanism B: Partial Profit ──────────────────────
        if cfg.get("partial_profit_enabled") and not forced_exit:
            _unreal_pp = (week_close - pos.entry_price) / pos.entry_price * 100 if pos.entry_price > 0 else 0
            if _unreal_pp >= cfg["partial_profit_trigger_pct"] and not pos.partial_taken:
                _sell_qty = math.floor(pos.qty * cfg["partial_profit_sell_pct"] / 100)
                if _sell_qty > 0 and _sell_qty < pos.qty:
                    _pp_proceeds = _sell_proceeds(_sell_qty, week_close, cfg)
                    cash += _pp_proceeds
                    pos.partial_sale_proceeds = _pp_proceeds
                    pos.partial_sale_qty = _sell_qty
                    pos.partial_sale_price = week_close
                    pos.partial_sale_date = wk
                    pos.qty -= _sell_qty
                    pos.partial_taken = True
                    _move = cfg.get("partial_profit_move_stop", "NONE")
                    if _move == "BREAKEVEN":
                        pos.stop_level = max(pos.stop_level, pos.entry_price)
                    elif _move == "HALF_BACK":
                        _hb = pos.entry_price + (pos.peak_close - pos.entry_price) / 2
                        pos.stop_level = max(pos.stop_level, _hb)
                    diag["partial_profit_taken"] += 1

        # ── Exit check (GTT trigger or forced exit) ──────────
        if forced_exit:
            exit_price = week_close
            _rem_proceeds = _sell_proceeds(pos.qty, exit_price, cfg)
            cash += _rem_proceeds
            proceeds = _rem_proceeds + pos.partial_sale_proceeds
            trade = _build_trade(pos, exit_price, wk, forced_exit_reason, proceeds)
            exit_regime = "unknown"
            ...
            trade["partial_sale_price"] = round(pos.partial_sale_price, 2) if pos.partial_sale_price > 0 else None
            trade["partial_sale_proceeds"] = round(pos.partial_sale_proceeds, 2) if pos.partial_sale_proceeds > 0 else None
...
def evaluate_entries_for_week(
    ...
    exit_ma_col: str,
    diag,
    sector_regime_lookup: dict | None = None,
    captier_regime_lookup: dict | None = None,
    ticker_metadata_lookup: dict | None = None,
    sector_rs_lookup: dict | None = None,
) -> tuple[list[Position], float]:
    """Per-week entry evaluation. Returns (updated_positions, updated_cash).

    diag is mutated in-place. Direct extraction of
    val_engine_b_experimental.py entry block (was lines 1091-1342).
    """
    # ── 6d. Process entries ──────────────────────────────────
    week_entries = entries_by_week.get(wk)
    if week_entries is not None and len(week_entries) > 0:
        # Look up overall regime
        regime = "unknown"
        breadth_pct = 0.0
        ...
        # Fast path: OVERALL_ONLY — identical to original locked Gladiator
        if not use_multi and not overall_allowed:
            diag["regime_blocked"] += len(week_entries)
        elif not use_multi and overall_allowed:
            ...
            candidates = candidates.sort_values(cfg.get("ranking_method", "rs_13w"), ascending=False, na_position="last")

            _rs_filter = cfg.get("sector_rs_filter", "NONE")
            _rs_lookup = sector_rs_lookup or {}
            _tm_fast = ticker_metadata_lookup or {}
            _wk_str_fast = str(wk)[:10] if _rs_filter != "NONE" else ""
            ...
            for _, entry_row in candidates.iterrows():
                ...
                if sector_counts.get(sector, 0) >= cfg["sector_limit"]:
                    diag["sector_limit"] += 1
                    continue
                # Sector RS filter (skip lookup when NONE)
                if _rs_filter != "NONE":
                    _tk = str(entry_row["ticker"])
                    _meta_f = _tm_fast.get(_tk, {})
                    _idx_f = _meta_f.get("index_symbol")
                    _q = _rs_lookup.get((_wk_str_fast, _idx_f)) if _idx_f else None
                    if not passes_sector_rs_filter(_q, cfg):
                        diag.setdefault("rs_filter_blocked", 0)
                        diag["rs_filter_blocked"] += 1
                        continue
                # Volume filter
                if cfg.get("volume_min_ratio") is not None:
                    _vr = entry_row.get("volume_ratio")
                    if pd.notna(_vr) and float(_vr) < cfg["volume_min_ratio"]:
                        diag.setdefault("volume_blocked", 0)
                        diag["volume_blocked"] += 1
                        continue
                ep = float(entry_row["entry_price"])
                sl = float(entry_row["stop_level"])
                # Structural stop for BREAKOUT entries
                if cfg.get("stop_method") == "STRUCTURAL":
                    _et = str(entry_row["entry_type"])
                    if _et.startswith("BREAKOUT"):
                        _ceil = entry_row.get("ceiling")
                        _atr = entry_row.get("atr_at_entry")
                        if pd.notna(_ceil) and pd.notna(_atr) and float(_atr) > 0:
                            _buf = cfg.get("stop_atr_buffer", 1.0)
                            if cfg.get("stop_atr_by_captier"):
                                _tk_s = str(entry_row["ticker"])
                                _ct_s = _tm_fast.get(_tk_s, {}).get("cap_tier", "MID")
                                _buf *= {"LARGE": 0.75, "MID": 1.0, "SMALL": 1.25, "MICRO": 1.25}.get(_ct_s, 1.0)
                            _struct = float(_ceil) - _buf * float(_atr)
                            if _struct < ep and _struct > 0:
                                sl = _struct
                rr = float(entry_row["rr_ratio"]) if not pd.isna(entry_row["rr_ratio"]) else 0
                # Pyramid: use reduced risk for probe
                _sizing_cfg = cfg
                if cfg.get("pyramid_enabled"):
                    _sizing_cfg = {**cfg, "risk_pct": cfg.get("pyramid_initial_risk_pct", 0.005)}
                sizing = compute_sizing(ep, sl, rr, equity_base, cash, _sizing_cfg)
                if sizing is None:
                    diag["sizing_fail"] += 1
                    continue
                ...
                # Pyramid: set probe fields
                if cfg.get("pyramid_enabled"):
                    _atr_e = float(entry_row.get("atr_at_entry", 0)) if pd.notna(entry_row.get("atr_at_entry")) else 0
                    pos.pyramid_status = "probe"
                    pos.pyramid_trigger_price = ep + cfg.get("pyramid_add_trigger_atr", 1.0) * _atr_e
                    pos.pyramid_atr_at_entry = _atr_e
                cash -= sizing["entry_cost"]
                positions.append(pos)
                sector_counts[sector] = sector_counts.get(sector, 0) + 1
                diag["entries"] += 1
        else:
            # Multi-regime path: per-ticker regime gate
            wk_str = str(wk)[:10]
            _sr = sector_regime_lookup or {}
            _cr = captier_regime_lookup or {}
            _tm = ticker_metadata_lookup or {}
            ...
            for _, entry_row in candidates.iterrows():
                ...
                s_regime = _sr.get((wk_str, idx_sym), "sideways") if idx_sym else "sideways"
                c_regime = _cr.get((wk_str, cap_tier), "sideways")
                ...
                # Sector RS filter
                _rs_filt = cfg.get("sector_rs_filter", "NONE")
                if _rs_filt != "NONE":
                    _rsl = (sector_rs_lookup or {}).get((wk_str, idx_sym)) if idx_sym else None
                    if not passes_sector_rs_filter(_rsl, cfg):
                        diag.setdefault("rs_filter_blocked", 0)
                        diag["rs_filter_blocked"] += 1
                        continue
                ...
                if cfg.get("stop_method") == "STRUCTURAL":
                    ...
                    if _et.startswith("BREAKOUT"):
                        ...
                        if pd.notna(_ceil) and pd.notna(_atr) and float(_atr) > 0:
                            _buf = cfg.get("stop_atr_buffer", 1.0)
                            if cfg.get("stop_atr_by_captier"):
                                _ct_s = cap_tier  # already available from regime lookup
                                _buf *= {"LARGE": 0.75, "MID": 1.0, "SMALL": 1.25, "MICRO": 1.25}.get(_ct_s, 1.0)
                            _struct = float(_ceil) - _buf * float(_atr)
                            if _struct < ep and _struct > 0:
                                sl = _struct
                ...
                _sizing_cfg2 = cfg
                if cfg.get("pyramid_enabled"):
                    _sizing_cfg2 = {**cfg, "risk_pct": cfg.get("pyramid_initial_risk_pct", 0.005)}
                sizing = compute_sizing(ep, sl, rr, equity_base, cash, _sizing_cfg2)
                if sizing is None:
                    diag["sizing_fail"] += 1
                    continue
                rs_val = float(entry_row["rs_13w"]) if not pd.isna(entry_row["rs_13w"]) else 0
                ...
                # Pyramid: set probe fields
                if cfg.get("pyramid_enabled"):
                    _atr_e2 = float(entry_row.get("atr_at_entry", 0)) if pd.notna(entry_row.get("atr_at_entry")) else 0
                    pos.pyramid_status = "probe"
                    pos.pyramid_trigger_price = ep + cfg.get("pyramid_add_trigger_atr", 1.0) * _atr_e2
                    pos.pyramid_atr_at_entry = _atr_e2
                cash -= sizing["entry_cost"]
                positions.append(pos)
                sector_counts[sector] = sector_counts.get(sector, 0) + 1
                diag["entries"] += 1

    # Surface per-stage rejection counters for live operability. When brain
    # runs this in-process the caller threads `diag` via shared dict and
    # logs through Python logging; the print here covers research-script
    # batch invocations where stdout IS the log.
    print(f"ENTRY_DIAG | as_of_date={str(wk)[:10]} | counters={dict(diag)}")
    return positions, cash
...
def run_simulation(
    ...
    # Phase 4 multi-regime data (pre-loaded dicts)
    sector_regime_lookup: dict | None = None,
    captier_regime_lookup: dict | None = None,
    ticker_metadata_lookup: dict | None = None,
    sector_rs_lookup: dict | None = None,
    ...
    for week_num, wk in enumerate(all_weeks):
        ...
        # ── 6a2. Pyramid add check (before exits) ─────────────────
        if cfg.get("pyramid_enabled"):
            _inv_now = sum(p.qty * p.current_close for p in positions)
            _eq_now = cash + _inv_now
            for pos in positions:
                if pos.pyramid_status != "probe":
                    continue
                if pos.current_close < pos.pyramid_trigger_price:
                    continue
                # Trigger hit
                if cfg.get("pyramid_add_sector_check"):
                    _wk_s = str(wk)[:10]
                    _tm_p = ticker_metadata_lookup or {}
                    _meta_p = _tm_p.get(pos.ticker, {})
                    _idx_p = _meta_p.get("index_symbol")
                    _sr_p = (sector_regime_lookup or {}).get((_wk_s, _idx_p), "sideways") if _idx_p else "sideways"
                    _rs_p = (sector_rs_lookup or {}).get((_wk_s, _idx_p)) if _idx_p else None
                    if _sr_p != "bull" or _rs_p not in ("TOP", "UPPER", None):
                        diag["pyramid_sector_blocked"] += 1
                        continue  # stay as probe, don't add
                # Compute add sizing
                add_price = pos.current_close
                sl_dist = add_price - pos.stop_level
                if sl_dist <= 0:
                    continue
                add_risk_amt = _eq_now * cfg.get("pyramid_add_risk_pct", 0.005)
                add_qty = math.floor(add_risk_amt / sl_dist)
                if add_qty <= 0:
                    continue
                add_cost = _buy_cost(add_qty, add_price, cfg)
                if add_cost > cash:
                    continue
                # Execute add
                cash -= add_cost
                ...
                old_qty = pos.qty
                pos.pyramid_original_qty = old_qty
                pos.pyramid_original_cost = pos.cost
                pos.pyramid_add_price = add_price
                pos.qty += add_qty
                pos.cost += add_cost
                # Blended entry price (VWAP of raw prices)
                pos.entry_price = (old_ep * old_qty + add_price * add_qty) / pos.qty
                pos.pyramid_status = "full"
                pos.pyramid_add_date = wk
                # Stop adjustment
                if cfg.get("pyramid_stop_on_add") == "BREAKEVEN":
                    original_ep = pos.pyramid_original_cost / pos.pyramid_original_qty / _buy_cost_factor(cfg) if pos.pyramid_original_qty > 0 else old_ep
                    pos.stop_level = max(pos.stop_level, original_ep)
                diag["pyramid_adds"] += 1

        # ── 6b. Process exits (BEFORE entries — Rule 1) ──────────
        positions, cash = evaluate_exits_for_week(
            wk=wk, positions=positions, cash=cash, cfg=cfg, cache=cache,
            regime_df=regime_df, regime_col=regime_col,
            closed_trades=closed_trades, diag=diag,
        )
        ...
        # ── 6d. Process entries ──────────────────────────────────
        positions, cash = evaluate_entries_for_week(
            wk=wk, positions=positions, cash=cash, equity_base=equity_base,
            entries_by_week=entries_by_week, cfg=cfg,
            regime_df=regime_df, regime_col=regime_col, breadth_col=breadth_col,
            exit_ma_col=exit_ma_col, diag=diag,
            sector_regime_lookup=sector_regime_lookup,
            captier_regime_lookup=captier_regime_lookup,
            ticker_metadata_lookup=ticker_metadata_lookup,
            sector_rs_lookup=sector_rs_lookup,
        )
    ...
    if cfg.get("force_close_at_end") and positions:
        for pos in positions:
            ...
            _rem_fc = _sell_proceeds(pos.qty, exit_price, cfg)
            cash += _rem_fc
            proceeds = _rem_fc + pos.partial_sale_proceeds
